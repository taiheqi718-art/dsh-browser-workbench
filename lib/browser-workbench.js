...
function extensionFor(tool, requested) {
    const ext = path.extname(requested);
    if (ext !== "")
        return ext;
    if (tool.endsWith("browser_pdf_save"))
        return ".pdf";
    if (tool.endsWith("browser_snapshot"))
        return ".md";
    if (tool.endsWith("browser_console_messages") || tool.includes("browser_network"))
        return ".log";
    if (tool.endsWith("browser_storage_state"))
        return ".json";
    return ".png";
}
...
function replaceFileResult(value, stage) {
...
function imageOf(value) {
    ...
    for (const block of result.content) {
        ...
        if (item.type !== "image" || typeof item.data !== "string")
            continue;
        if (item.mimeType !== "image/png" && item.mimeType !== "image/jpeg" && item.mimeType !== "image/webp")
            continue;
        return { mimeType: item.mimeType, bytes: Buffer.from(item.data, "base64") };
    }
    return undefined;
}
function normalizeRequestPolicy(policy) {
    ...
    if (policy.blockAll === true && origins.length !== 0)
        throw new Error("blockAll cannot also allow origins");
    if (origins.length === 0 && policy.blockAll !== true)
        throw new Error("browser request policy requires at least one exact origin");
    return { allowedOrigins: origins, ...(policy.blockAll === true ? { blockAll: true } : {}) };
}
function policyInstallerCode(allowedOrigins, marker) {
 ...
 */
export function serveBrowserWorkbench(host, config, note) {
    ...
    const touch = (sessionId, patch) => {
        ...
        if (pending !== undefined) {
            for (const waiter of [...pending]) {
                if (waiter.afterRevision === next.revision)
                    continue;
    ...
    const snapshotOf = (sessionId, afterRevision) => {
    ...
    const safeOutputTarget = async (workspace, requested) => {
        ...
        try {
            ...
            if (existing.isSymbolicLink())
                throw new Error(`browser output target is a symbolic link: ${requested}`);
            if (existing.isDirectory())
                throw new Error(`browser output target is a directory: ${requested}`);
        }
        catch (error) {
    ...
    const safeInputSource = async (workspace, requested) => {
        const checked = resolveTarget(workspace, requested);
        if (!checked.insideWorkspace || checked.unresolved === true) {
        ...
        if (!info.isFile())
            throw new Error(`browser input path is not a regular file: ${requested}`);
    ...
    const stageName = (suffix) => path.join(outputDir, `.dsh-browser-${randomUUID()}${suffix}`);
    const track = (file) => {
    ...
    const stageInput = async (workspace, requested) => {
    ...
    const capture = async (group, exec, sessionId, workspace) => {
        if (!liveCapture || disposed || group.screenshot === undefined)
            return;
    ...
    const prepare = async (tool, args, exec) => {
        const original = recordOf(args);
        const workspace = workspaceOf(exec);
        const next = { ...original, _meta: { cwd: workspace } };
        const stagedInputs = [];
        ...
        if (tool === NAVIGATE_TOOL && typeof original.url === "string"
            ...
            let localFile;
            try {
                localFile = original.url.startsWith("file:") ? fileURLToPath(original.url) : original.url;
            }
            catch {
                throw new Error("browser local preview requires a valid file:// URL");
            }
    ...
    tools.register = function registerBrowserAware(definition) {
        ...
        while (Object.hasOwn(owner, Symbol.for("cordis.shadow"))) {
            const caller = Object.getPrototypeOf(owner);
            if (caller === null)
                break;
            owner = caller;
        }
        ...
        const originalDefinition = { ...definition, execute: originalExecute };
        if (definition.name === SCREENSHOT_TOOL)
            group.screenshot = originalDefinition;
        if (definition.name === UNSAFE_CODE_TOOL)
            group.unsafeCode = originalDefinition;
        const wrapped = {
            ...definition,
            ...
            async execute(args, exec) {
                ...
                try {
                    ...
                    if (definition.name === VIDEO_START_TOOL) {
                        const requested = typeof recordOf(args).filename === "string" ? recordOf(args).filename : undefined;
                        if (requested !== undefined && requested !== "") {
                            if (sessionId === undefined)
                                throw new Error("browser video output requires a calling Session");
                    ...
                    if (sessionId !== undefined) {
                        ...
                        if (directImage !== undefined && directImage.bytes.byteLength <= maxFrameBytes) {
                            ...
                            if (CAPTURE_AFTER.has(definition.name) && workspace !== undefined) {
                                await capture(group, exec, sessionId, workspace);
                            }
                        }
                    }
                    return value;
                    ...
                    if (output !== undefined)
                        await clean(output.staged);
                    for (const file of stagedInputs)
                        await clean(file);
                }
            },
        };
    ...
    return {
        snapshot(sessionId, afterRevision) {
            return snapshotOf(sessionId, afterRevision);
        },
        async waitSnapshot(sessionId, afterRevision, timeoutMs) {
            const current = snapshotOf(sessionId, afterRevision);
            if (current.revision !== afterRevision || timeoutMs <= 0 || disposed)
                return current;
            const bounded = Math.min(30_000, Math.max(250, timeoutMs));
            return await new Promise((resolve) => {
                const bucket = waiters.get(sessionId) ?? new Set();
        ...
        setRequestPolicy(sessionId, policy) {
            if (policy === null)
                requestPolicies.delete(sessionId);
            else {
        ...
        dispose() {
            ...
            if (previousOwnRegister !== undefined)
                Object.defineProperty(tools, "register", previousOwnRegister);
            else if (!hadOwnRegister)
                delete tools.register;
...
//# sourceMappingURL=browser-workbench.js.map