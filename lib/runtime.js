 ...
 */
export function createBrowserMcpDescriptor(config) {
    ...
    const executable = requireAbsoluteExisting("browser executable", config.browserExecutable
        ?? browserCandidates().find((candidate) => candidate !== undefined && existsSync(candidate))
        ?? "");
    ...
    if (!/^\d{2,5}x\d{2,5}$/.test(viewport))
        throw new Error(`invalid browser viewport: ${viewport}`);
    return {
        serverName: "browser",
        transport: "stdio",
