...
- Responsive-layout retest in the packaged profile: the normal browser surface occupied a real right-hand grid track and the conversation column reflowed instead of being covered. With the project sidebar expanded, workspace fullscreen started at its measured 280 px boundary; collapsing the sidebar during fullscreen moved that boundary to 56 px without leaving a gap. `Esc` returned to the wide side panel without reloading or losing the latest frame.
- Address-field retest (2026-09-07) in the packaged profile: a Session-local `index.html` still navigated through its capability-addressed loopback preview, while the visible browser chrome showed `workspace/index.html` instead of the private port and token. Ordinary HTTP(S) snapshots retain the browser's reported final URL.
