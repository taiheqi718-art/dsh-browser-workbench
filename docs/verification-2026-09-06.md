...
- Responsive-panel retest in the packaged profile: the normal browser surface opened at roughly half of the available viewport, the expand control covered the full workbench viewport, and `Esc` returned to the wide side panel without reloading or losing the latest frame.
