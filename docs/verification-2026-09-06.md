...
- Reopen lifecycle retest in the same isolated full profile: the first browser action opened the right details track, the panel close button folded it without losing the captured frame, the new per-session **Browser** header control remained visible, and clicking it restored the same live view. Production port `3080` was not used.
