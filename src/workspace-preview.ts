...
export function createWorkspacePreviewBridge(
  ...
  const start = async (): Promise<void> => {
    ...
    if (disposed) throw new Error("browser workspace preview is disposed");
  ...
  return {
    async urlForFile(workspace: string, absoluteFile: string): Promise<string> {
      ...
      const info = await stat(checked.resolved);
      if (!info.isFile()) throw new Error("browser preview target is not a regular file");
      if (info.size > maxFileBytes) throw new Error(`browser preview file exceeds the ${maxFileBytes}-byte limit`);
      await start();
